...
if (empty($firstname) && empty($lastname) && empty($birthday) && empty($country) && empty($serie) && empty($sexe) && empty($password) && empty($image)) {
    header("../views/register.php?error=0");
}
if (empty($firstname)) {
    header("../views/register.php?error=1");
}
if (empty($lastname)) {
    header("../views/register.php?error=2");
}
if (empty($birthday)) {
    header("../views/register.php?error=3");
}
if (empty($anne_bac)) {
    header("../views/register.php?error=4");
}
if (empty($country)) {
    header("../views/register.php?error=5");
}
if (empty($serie)) {
    header("../views/register.php?error=6");
}
if (empty($sexe)) {
    header("../views/register.php?error=7");
}
if (empty($password)) {
    header("../views/register.php?error=8");
}
if (empty($image)) {
    header("../views/register.php?error=9");
}
...
if (file_exists($targetFile)) {
    $uploadOk = 0;
    header("../views/register.php?error=10");
}
...
if ($_FILES["image"]["size"] > 2000000) {
    ...
    $uploadOk = 0;
    header("../views/register.php?error=11");
}

// Autoriser certains formats de fichiers
$allowedExtensions = array("jpg", "jpeg", "png", "gif");
if (!in_array($imageFileType, $allowedExtensions)) {
    header("../views/register.php?error=12");
    $uploadOk = 0;
}
